...
class Meteor:
    ...
    def __init__(self):
        ...
        meteor_cmd = ['java', '-jar', '-Xmx{}'.format(mem), METEOR_JAR, '-', '-', '-stdio', '-l', 'en', '-norm']
        env = os.environ.copy()
    ...
    def compute_score(self, references, candidates):
        ...
        Args:
            references (list): list of references, each reference should be tokenized into a list of tokens
            candidates (list): list of candidates, each candidate should be tokenized into a list of tokens

        Returns:
            (float, list):
                - average METEOR score
                - list of scores of each sample
        """
        scores = []
        eval_line = 'EVAL'
        ...
        with self.lock:
            ...
            for reference, candidate in zip(references, candidates):
                if len(candidate) * len(reference) == 0:
                ...
                stat = self._stat(candidate, [reference])
